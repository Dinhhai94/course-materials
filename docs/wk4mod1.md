...
1. Make sure that you have functional components that can be rendered with a single `render` function. Something like:
...
1. At this point, `Content` should be able to handle the Array of `books`. Last time, we didn't know about loops of any kind, so the `Book` component looked something like:
...
1. We can do the same thing with the `sellingPoints` Array in our `Book` component. HINT:
...
1. Our last iteration of the form `submit` event handler was a bit repetetive. How can it be refactored to make our `newProduct` construction a bit more `DRY`? HINT:
...
1. Now let's let users filter between Books and Albums when they click on the `books` and `albums` links in the `Navigation` component. What might that look like? HINT:
...
2. Can you filter `products` by `title` using a Search `input` and a `keyup` event listener?
3. Can you delete books from the front end on `click`?
4. Can you delete books from the back-end database using a `DELETE` request \(e.g. `axios.delete`\)?
5. Can you add the ability to edit books on the front end?
6. Can you persist book edits to the API using a `PATCH` request \(e.g. `axios.patch`\)?
7. Can you add loading indicators using Font Awesome spinners and fa-spin?
