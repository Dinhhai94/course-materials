# Local Deve Environment and First Steps
...
Please reference the Savvy Coders notebook and go through our [video series](https://www.notion.so/codefinity/Setting-up-a-Local-Dev-Environment-97de62a546f943fbbc7a246b4785a343) that before reading through the rest of this.
...
3. Also in your CLI, use the command `touch intro.md` to create a new _MarkDown_ file named inside of `FirstnameLastname` directory.
4. You can open specific directories in VSCode using the `code .` command from your command line! Here, the `.` is an _argument_ that we are passing to the `code` command that reinforces that we want VS Code to open up directly in our current directory.
